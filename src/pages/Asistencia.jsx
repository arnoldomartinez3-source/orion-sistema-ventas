...
export default function Asistencia({ empleados = [] }) {
  ...
  const filas = useMemo(() => {
    ...
    return dias.map(fecha => {
      ...
      const estado = entrada && salida ? 'completo' : entrada ? 'sinsalida' : 'sinmarca'
  ...
  const anomalias = filas.filter(f => f.estado === 'sinsalida').length
  ...
  return (
    <>
      ...
      <div className="asis-stats">
        ...
        <div className="asis-stat"><div className="v" style={{ color: '#d98a00' }}>{anomalias}</div><div className="l">Sin salida</div></div>
      ...
      {detalle && (
        <div className="modal-overlay" onClick={() => setDetalle(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 560 }}>
            ...
            {/* Marcaciones del día con fotos */}
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', margin: '8px 0 18px' }}>
              {['entrada', 'salida'].map(t => {
                const m = t === 'entrada' ? detalle.entrada : detalle.salida
                return (
                  <div key={t} style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 6 }}>{t}</div>
                    {m
                      ? <>
                          {m.fotoUrl ? <img className="asis-foto" src={m.fotoUrl} alt={t} /> : <div className="asis-foto" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 11 }}>sin foto</div>}
                          <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, marginTop: 6 }}>{horaDe(m.timestamp)}</div>
                        </>
                      : <div className="asis-foto" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 12 }}>—</div>}
                  </div>
                )
              })}
            </div>
