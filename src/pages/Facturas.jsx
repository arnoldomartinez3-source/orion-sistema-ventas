...
const TIPOS_DTE = [
  ...
  { codigo: 'CCF', nombre: 'Comprobante de Crédito Fiscal', desc: 'Para empresas con NRC', color: '#4f8cff' },
  { codigo: 'NC', nombre: 'Nota de Crédito', desc: 'Anular o ajustar factura', color: '#f59e0b' },
  { codigo: 'ND', nombre: 'Nota de Débito', desc: 'Cobros adicionales', color: '#8b5cf6' },
  { codigo: 'FEX', nombre: 'Factura de Exportación', desc: 'Ventas al extranjero', color: '#ec4899' },
  { codigo: 'NR', nombre: 'Nota de Remisión', desc: 'Envío sin cobro', color: '#6b7280' },
]
...
const factStyles = `
  ...
  @media (max-width: 480px) { .fact-resumen { grid-template-columns: 1fr 1fr; } }
  ...
  .filter-tab {
    ...
    border: 1.5px solid var(--border); background: transparent; font-family: var(--font);
  }
  ...
  .tipo-tag {
    ...
    border-radius: 6px; font-family: var(--mono); letter-spacing: 0.5px;
    border: 1.5px solid;
  }
  ...
  .fact-detalle-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; flex-wrap: wrap; gap: 12px; }
...
export default function Facturas() {
  ...
  const calcularIva = (subtotal) => {
    ...
    const iva = s * 0.13
    const total = s + iva
    setForm(f => ({ ...f, subtotal: subtotal, iva: iva.toFixed(2), total: total.toFixed(2) }))
  }

  const filtradas = facturas.filter(f => {
    const coincide = f.cliente?.toLowerCase().includes(busqueda.toLowerCase()) ||
      f.numero?.toLowerCase().includes(busqueda.toLowerCase()) ||
      f.nit?.includes(busqueda)
  ...
  const abrirModal = (factura = null) => {
    if (factura) {
      ...
      setForm({ ...emptyForm, numero: `FE-${String(num).padStart(6, '0')}` })
    }
    setModalOpen(true)
  }
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    try {
      await updateDoc(doc(db, 'facturas', id), { estadoPago: nuevoEstado, updatedAt: serverTimestamp() })
    } catch (e) {
      ...
      <div className="topbar">
        <div style={{ paddingLeft: 50 }}>
          ...
          <div className="page-sub" style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
            {facturas.length} documentos emitidos
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>EMISIÓN</th><th>VENCE</th><th>ESTADO PAGO</th><th>ACCIONES</th>
                </tr>
              </thead>
              <tbody>
                {filtradas.length === 0 ? (
                  ...
                  return (
                    <tr key={f.id}>
                      ...
                      <td style={{ color: f.fechaVencimiento ? 'var(--accent3)' : 'var(--muted)', fontSize: 12 }}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button className="btn btn-ghost btn-sm" onClick={() => setDetalleOpen(f)}>👁️</button>
                          <button className="btn btn-ghost btn-sm" onClick={() => abrirModal(f)}>✏️</button>
                          <button className="btn btn-danger btn-sm" onClick={() => eliminar(f.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  )
          ...
          <div className="modal modal-xl" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div className="form-group">
                <label className="form-label">NOMBRE / RAZÓN SOCIAL <span style={{ color: 'var(--danger)' }}>*</span></label>
              ...
              <div className="form-group">
                ...
                <input className="input" placeholder="Dirección del cliente" value={form.direccion} onChange={e => setForm(f => ({ ...f, direccion: e.target.value }))} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div className="form-group">
                <label className="form-label">DESCRIPCIÓN DE BIENES O SERVICIOS</label>
                <input className="input" placeholder="Ej: Venta de artículos de oficina" value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} />
              </div>

              <div className="form-group">
                <label className="form-label">SUBTOTAL (SIN IVA) $</label>
                <input
                  className="input" type="number" step="0.01" placeholder="0.00"
                  value={form.subtotal}
              ...
              {form.subtotal && (
                <div className="iva-calc">
                  <div className="iva-row"><span style={{ color: 'var(--muted)' }}>Subtotal</span><span className="amount">{fmt(parseFloat(form.subtotal) || 0)}</span></div>
                  <div className="iva-row"><span style={{ color: 'var(--muted)' }}>IVA (13%)</span><span className="amount" style={{ color: 'var(--accent3)' }}>{fmt(parseFloat(form.iva) || 0)}</span></div>
                  <div className="iva-row total"><span>TOTAL</span><span className="amount" style={{ color: 'var(--accent)' }}>{fmt(parseFloat(form.total) || 0)}</span></div>
                </div>
              )}
            </div>

            {/* PAGO Y FECHAS */}
            <div className="modal-section">PAGO Y FECHAS</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div className="form-grid">
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input className="input" placeholder="Notas opcionales..." value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))} />
              </div>
            </div>
              ...
              return (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
                    ...
                    <span className={`estado-pago ${f.estadoPago}`}><span className="dot" style={{ width: 7, height: 7, borderRadius: '50%', background: 'currentColor', display: 'inline-block' }} /> {f.estadoPago.charAt(0).toUpperCase() + f.estadoPago.slice(1)}</span>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
                    {[
                      { label: 'No. DTE', value: f.numero, mono: true },
                      { label: 'Fecha Emisión', value: formatFecha(f.fechaEmision) },
                      ...
                      <div key={item.label}>
                        <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 3 }}>{item.label}</div>
                        <div style={{ fontSize: 14, fontWeight: 600, fontFamily: item.mono ? 'var(--mono)' : 'var(--font)' }}>{item.value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {f.descripcion && (
                    <div style={{ marginBottom: 16, padding: '12px 16px', background: 'var(--surface2)', borderRadius: 10, border: '1px solid var(--border)' }}>
                      <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, marginBottom: 4 }}>DESCRIPCIÓN</div>
