...
const sidebarStyles = `
  ...
  @media (max-width: 768px) {
    ...
    .sidebar.mobile-open { transform: translateX(0); box-shadow: 8px 0 40px rgba(0,0,0,0.4); }
  }
  ...
  .sidebar-logo {
    ...
    min-height: 80px; overflow: hidden; position: relative;
  ...
  .sidebar-logo-mini {
    ...
    box-shadow: 0 4px 14px rgba(27,46,107,0.4);
    overflow: hidden;
  }
  ...
  .nav-item.active { background: rgba(46,111,212,0.12); }
  .nav-item.active::before {
    content: ''; position: absolute; left: 0; top: 20%; bottom: 20%;
    width: 3px; background: #2E6FD4; border-radius: 99px;
  }
  ...
  .nav-item.active .nav-icon-wrap { background: rgba(46,111,212,0.15); }

  .nav-label { font-size: 14px; font-weight: 600; color: var(--text2); transition: all 0.18s; overflow: hidden; }
  .nav-item.active .nav-label { color: #2E6FD4; font-weight: 700; }
  ...
  .logout-modal-title { font-size: 18px; font-weight: 800; margin-bottom: 8px; }
...
const OrionMini = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500" width="30" height="30">
    <defs>
      <filter id="mGG" x="-150%" y="-150%" width="400%" height="400%">
        <feGaussianBlur stdDeviation="12" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
      </filter>
      <filter id="mGB" x="-150%" y="-150%" width="400%" height="400%">
        <feGaussianBlur stdDeviation="14" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
      </filter>
      <filter id="mGT" x="-150%" y="-150%" width="400%" height="400%">
        <feGaussianBlur stdDeviation="10" result="b"/>
        <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
      </filter>
      <radialGradient id="mGGG" cx="40%" cy="35%" r="60%">
        <stop offset="0%" stopColor="#fff"/><stop offset="40%" stopColor="#2EECC5"/><stop offset="100%" stopColor="#00B89F"/>
      </radialGradient>
      <radialGradient id="mGGB" cx="40%" cy="35%" r="60%">
        <stop offset="0%" stopColor="#fff"/><stop offset="40%" stopColor="#5AC8F5"/><stop offset="100%" stopColor="#1E7FBA"/>
      </radialGradient>
      <radialGradient id="mGGT" cx="40%" cy="35%" r="60%">
        <stop offset="0%" stopColor="#fff"/><stop offset="40%" stopColor="#35C4D8"/><stop offset="100%" stopColor="#0E87A8"/>
      </radialGradient>
      <linearGradient id="mLM" x1="0%" y1="100%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#1A6FA3" stopOpacity="0.9"/><stop offset="100%" stopColor="#00B89F" stopOpacity="0.9"/>
      </linearGradient>
      <linearGradient id="mLB" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#1A6FA3" stopOpacity="0.85"/><stop offset="100%" stopColor="#35C4D8" stopOpacity="0.85"/>
      </linearGradient>
      <linearGradient id="mLR" x1="0%" y1="100%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#35C4D8" stopOpacity="0.85"/><stop offset="100%" stopColor="#00B89F" stopOpacity="0.85"/>
      </linearGradient>
    </defs>
    <line x1="148" y1="268" x2="292" y2="268" stroke="url(#mLB)" strokeWidth="3" strokeLinecap="round"/>
    <line x1="148" y1="268" x2="310" y2="118" stroke="url(#mLM)" strokeWidth="3" strokeLinecap="round"/>
    <line x1="292" y1="268" x2="310" y2="118" stroke="url(#mLR)" strokeWidth="2.8" strokeLinecap="round"/>
    <circle cx="310" cy="118" r="17" fill="url(#mGGG)" filter="url(#mGG)"/>
    <circle cx="148" cy="268" r="19" fill="url(#mGGB)" filter="url(#mGB)"/>
    <circle cx="292" cy="268" r="14" fill="url(#mGGT)" filter="url(#mGT)"/>
  </svg>
)
...
export default function Sidebar() {
  ...
  const getIniciales = () => {
    if (!user) return 'U'
    return (user.displayName || user.email || 'U').split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)
  }
        ...
        <div className="sidebar-logo">
          {collapsed ? (
            ...
            <div className="sidebar-logo-full">
              <OrionLogo width={160} textColor={dark ? '#ffffff' : '#0C2461'} />
            </div>
          )}
