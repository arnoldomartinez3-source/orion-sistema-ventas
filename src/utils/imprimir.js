...
export const generarPDF = async (fOriginal, empresaOriginal = {}) => {
  ...
  // como en la factura de consumidor final de siempre, el IVA NO se desglosa en la
  // impresión (sí va en el JSON: ivaItem/totalIva; el MH no fija qué líneas lleva la
  // representación gráfica). CCF, NC y ND sí muestran su IVA.
...
<div class="page contenido">
  ...
  <div class="bloque-inferior">
    ...
    <div class="bloque-totales">
      ...
      ${ivaIncluido ? '' : `<div class="bloque-totales-fila"><span>IVA 13%:</span><span>${fmt(ivaCalculado)}</span></div>`}
...
export const generarTicket = async (fOriginal, empresaOriginal = {}) => {
  ...
  const reteRenta = resOf?.reteRenta ?? 0
  const totalPagar = resOf?.totalPagar ?? f.totalPagar ?? (f.total || 0)
  // FE con montos con IVA incluido → Sub Total == Total; el IVA no se imprime (ver arriba).
...
/* Térmicas 203 dpi: Courier New normal (trazo finísimo) se pierde al imprimir, y
   TODO en negrita se empasta. Lo que sale bien (igual que la página de prueba de
   Windows): sans-serif normal, 12 px, negro puro; negrita solo en títulos/totales. */
body{font-family:'Segoe UI',Arial,Helvetica,sans-serif;font-weight:400;width:72mm;font-size:12px;color:#000;padding:3mm;line-height:1.35;background:#fff;margin:24px auto;box-shadow:0 6px 24px rgba(0,0,0,0.15);min-height:auto;border-radius:2px;}
.c{text-align:center;}
.b{font-weight:bold;}
...
.row{display:flex;justify-content:space-between;gap:8px;font-size:11.5px;margin:1px 0;}
.row .lbl{font-weight:700;}
.row .val{text-align:right;word-break:break-all;}
.row-mono{font-family:'Lucida Console',Consolas,'Courier New',monospace;font-weight:400;font-size:10.5px;}
...
.item-detalle{display:flex;justify-content:space-between;font-size:11.5px;color:#000;}
.tot-row{display:flex;justify-content:space-between;font-size:12px;margin:1px 0;}
.tot-row.fin{font-size:15px;font-weight:900;border-top:2px solid #000;padding-top:4px;margin-top:3px;}
.qr-box{text-align:center;margin:6px 0;}
.qr-box img{width:42mm;max-width:100%;}
.qr-leyenda{font-size:10.5px;text-align:center;margin-top:2px;line-height:1.3;}
.pie{font-size:10.5px;text-align:center;color:#000;line-height:1.4;}
